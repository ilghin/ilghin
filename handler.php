<?php
	...
	if ($_POST['action'] === 'update') {
        ...
        foreach($XML->owl_Class as $owl_Class) {
            if ($owl_Class->rdfs_subClassOf['rdf_resource'] == PREFIX . $_POST['id']) {
                $rdfs_subClassOf = $owl_Class->rdfs_subClassOf;
                $DOM = dom_import_simplexml($rdfs_subClassOf);
                $DOM->setAttribute('rdf_resource', PREFIX . $_POST['rdf_about']);
            }
        }
		...
		echo "Определение $rdf_About обновлено.";
	}
?>